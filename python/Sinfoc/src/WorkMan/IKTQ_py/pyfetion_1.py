...
if(proxy_str):
	...
	print {"http":"http://%(user)s:%(pass)s@%(host)s:%(port)d" % proxy_info}
else:
	proxy_info = False
...
def http_send(url, body="", exheaders="", login=False):
    ...
    if proxy_info:
    	http_p = "%(host)s:%(port)d" % proxy_info; 
    	print   http_p     
...
def main(argv=None):
    ...
    try:
        ...
        print "Error fetion num."
        return 1
    except PyFetionAuthError, e:
        print "Error cell num"
        return 2
    ...
    ret = phone.send_sms("I", "13581927326", True)
