...
class fetion_recv(Thread):
    ...
    def run(self):
        ...
        s = {"PC":"电脑","WAP":"WAP","Symbian":"S60","PHONE":"手机"}
    ...
    def cmd(self,arg):
        ...
        if cmd[0] == "":
            ...
            for i in c:
                if c[i][0] == '':
                    c[i][0] = i[4:4+9]
            printl("%-4s%-37s%-13s%-20s" % ('NO.','飞信号','手机号','昵称'))
            printl("-"*64)
            printl(status[FetionOnline])
            for i in range(num):
                if c[c.keys()[i]][2] != FetionHidden and c[c.keys()[i]][2] != FetionOffline:
                    printl("%-4d%-37s%-13s%-20s" % (i,c.keys()[i],c[c.keys()[i]][1],c[c.keys()[i]][0]))
            ...
            global config_file
            config = FetionConfig()
            config.loginuser = self.phone.sid
            config.loginpass = self.phone.passwd
...
class FetionConfig:
    ...
    def __init__(self):
        """初始化
        """
        self.key = des("DESCRYPT", CBC, "\6\1\1\6\1\3\2\3", pad=None, padmode=PAD_PKCS5)
        pass

    def Load(self, config_file = 'fetion.conf'):
        """
        读取配置文件
        ...
        try:
            ...
            full = fp.read()
            result = re.findall('LOGINUSER=(\d+)', full)
            if len(result) >= 1:
                self.loginuser = result[0]

            result = re.findall('LOGINPASS=([^\r\n\t]+)', full)
            if len(result) >= 1:
                self.loginpass = self.Decode(result[0])

        except Exception, e:
            printl('Get config error!\n%s' % str(e))
            result = False
        finally:
            if fp:
                fp.close()
    ...
    def Save(self, config_file = 'fetion.conf'):
        """
        保存配置文件
        @param config_file: 配置文件名
        """
        ...
        try:
            lines = []
            lines.append('[CONFIG]\r\n')
            lines.append('LOGINUSER=%s\r\n' % self.loginuser)
            lines.append('LOGINPASS=%s\r\n' % self.Encode(self.loginpass))

            fp = open(config_file, 'w+')
            fp.writelines(lines)
        except Exception,e:
            printl('Save config error!\n%s' % str(e))
            result = False
        finally:
            if fp:
                fp.close()
    ...
    def Decode(self, value):
        try:
            return self.key.decrypt(base64.decodestring(value))
        except:
            return ''

    def Encode(self, value):
        try:
            return base64.encodestring(self.key.encrypt(value))
        except Exception,e:
            print e
            return ''
